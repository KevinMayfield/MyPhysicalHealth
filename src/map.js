...
const LOW_VALUE_COLOR = '#6f6c5f';
const JUNCTION_COLOR = '#8a5a2f';
const BONK_COLOR = '#7a2e3f';
...
// A small filled circle with a minus sign: a stretch that sat in the
// easiest zone for no obvious terrain reason.
function lowValueMarkup(x, y) {
  const r = 7;
  return [
    `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r}" fill="${LOW_VALUE_COLOR}" stroke="#fffaf0" stroke-width="2" />`,
    `<text x="${x.toFixed(1)}" y="${(y + 4).toFixed(1)}" font-family="Arial, Helvetica, sans-serif" font-size="13" font-weight="800" fill="#fffaf0" text-anchor="middle">&#8722;</text>`,
  ].join('');
}

// A small warning triangle: a low-value stretch where speed also dipped
// sharply, hinting at a junction, lights, gate, or a rough patch.
function junctionMarkup(x, y) {
  ...
  const points = [
    ...
    .join(' ');
  return [
    `<polygon points="${points}" fill="${JUNCTION_COLOR}" stroke="#fffaf0" stroke-width="1.5" stroke-linejoin="round" />`,
    `<text x="${x.toFixed(1)}" y="${(y + s * 0.75 - 2).toFixed(1)}" font-family="Arial, Helvetica, sans-serif" font-size="11" font-weight="800" fill="#fffaf0" text-anchor="middle">!</text>`,
  ].join('');
}

// A diamond with a downward arrow: an extended, non-recovering energy
// crash — distinct in shape and colour from the low-value/junction dots.
function bonkMarkup(x, y) {
  ...
  const points = [
    ...
    .join(' ');
  return [
    `<polygon points="${points}" fill="${BONK_COLOR}" stroke="#fffaf0" stroke-width="1.5" stroke-linejoin="round" />`,
    `<text x="${x.toFixed(1)}" y="${(y + 4).toFixed(1)}" font-family="Arial, Helvetica, sans-serif" font-size="11" font-weight="800" fill="#fffaf0" text-anchor="middle">&#8595;</text>`,
  ].join('');
}
 ...
 * schematic renderer.
 */
async function renderBasemap(points, segments, effortSegments, highlights, lowValueSpots, bonkEpisodes) {
 ...
 */
function renderSchematic(points, segments, effortSegments, highlights, lowValueSpots, bonkEpisodes) {
  const width = 900;
  const height = 560;
  const margin = 60;
...
async function renderRouteMap(points, segments, effortSegments, highlights, lowValueSpots, bonkEpisodes) {
  try {
    return await renderBasemap(points, segments, effortSegments, highlights, lowValueSpots, bonkEpisodes);
  } catch {
    return renderSchematic(points, segments, effortSegments, highlights, lowValueSpots, bonkEpisodes);
  }
}
